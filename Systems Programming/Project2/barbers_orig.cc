...
void Shop::run() {
  ...
  for (int next_customer_id = 0; ; next_customer_id++) 
  {
    ...
    cout << "wait before the next customer" << endl;
  ...
  // vector<Barber*> sleepingBarbers;
  // {
  //   Lock lock(mutex);
  //   isShopClosed = true;
  //   #if defined(VERBOSE)
  //   cout << "@@@@@ " << barber_queue.size() << " barbers are still sleeping @@@@@" << endl;
  //   #endif
  //   while(!barber_queue.empty()){
  //     sleepingBarbers.push_back(barber_queue.front());
  //     barber_queue.pop();
  //   }
  // }

  // for(auto barber : sleepingBarbers)
  // {
  //   #if defined(VERBOSE)
  //   cout << "waking up barber" << barber->id << " for shop close" << endl;
  //   #endif
  //   barber->wakeup();
  // }
...
void Barber::run() 
{
  ...
  while(true)
  {
    ...
    {
      if(!shopClosed)//shop is still open so sleep
        shop->barberSleepInSeat(this);
      else{//shop closed
        cout << "barber(" << id << ") leaves for home" << endl;
        break;      
      }
      cout << "customer[" << myCustomer->id << "] wakes barber(" << id << ")" << endl;
    }else{//found a customer in a waiting room
    ...
    myCustomer->wakeup();//let the customer go(wake up the customer thread)
  }
}
...
void Customer::run() 
{
  ...
  if(bw.barber != nullptr)//barber is available
  {
    next_customer(bw.barber);

    bw.barber->awaken(this);//wake up a barber

    #if defined(VERBOSE)
      shop->showSleepingBarbers();
    #endif
  ...
  {
    #if defined(VERBOSE)
      cout << "There is a chair for this customer in a waiting room." << endl;
    #endif
  ...
  cout << "customer[" << id << "] leaves statisfied" << endl;
}
